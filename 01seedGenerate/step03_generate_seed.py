...
def print_or_write(args, start_index, total_iterations=1):
    ...
    for i in tqdm(range(total_iterations), desc="Generating", leave=True):
        iteration_dir = f"{base_dir}/iteration_{i}"
        os.makedirs(iteration_dir, exist_ok=True)

        generated_sequences = generate(args)
        if print_or_write == 0:
            for seq in generated_sequences:
                print(''.join(seq))
        elif print_or_write == 1:
            for index, value in enumerate(generated_sequences, start=start_index):
                file_name = f"MyJVMTest_{index}.java"
                file_path = os.path.join(iteration_dir, file_name)
                with open(file_path, "w") as f:
                    f.write(value.replace("MyJVMTest", f"MyJVMTest_{start_index}"))
